...
animals.post("/add-animal", async (req, res) => {
  ...
  if (!animalName)
    return res.status(400).send({ error: "Animal name is required" });
  if (!animalType)
    return res.status(400).send({ error: "Animal type is required" });
  if (!incident) return res.status(400).send({ error: "Incident is required" });
  if (!healthStatus)
    return res.status(400).send({ error: "Health status is required" });
  ...
  try {
    const results = await pool.query(
      "INSERT INTO animals (animal_name, animal_type, incident, health_status, latitude, longitude) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *",
      [animalName, animalType, incident, healthStatus, latitude, longitude]
    );
    return res
      .status(200)
      .json({ message: "Animal added successfully!", results: results.rows });
  } catch (err) {
    return res.status(500).json({ error: err });
  }
});
...
animals.patch("/:animalId/update", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Animal updated successfully!", results: results.rows });
  } catch (error) {
...
animals.delete("/found/:animalId", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Animal has been found!", results: results.rows });
  } catch (error) {
    res.status(500).json({ error: error });
  }
});
...
animals.delete("/found/all", async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Found all animals!", results: results.rows });
  } catch (error) {
    res.status(500).json({ error: error });
  }
});

...
/**
 * Calculate the adjusted radius for the area circle based on sightings
 */
export const calculateAdjustedRadius = (sightings, animalType) => {
  if (!sightings || sightings.length < 2) {
    // Return default radius based on animal type if not enough sightings
    return MAX_ROAMING_RANGES[animalType] || MAX_ROAMING_RANGES.other;
  }

  const center = calculateCenter(sightings);
  const distances = sightings.map((s) => calculateDistance(center, s));

  // Calculate average and maximum distances from center
  const maxDistance = Math.max(...distances);
  const avgDistance = distances.reduce((a, b) => a + b, 0) / distances.length;

  // Use the larger of average distance or half of max distance as base
  const baseRadius = Math.max(avgDistance, maxDistance / 2);

  // Apply maximum range limit for the animal type
  const maxRange = MAX_ROAMING_RANGES[animalType] || MAX_ROAMING_RANGES.other;

  // Return the smaller of calculated radius or maximum range
  return Math.min(baseRadius, maxRange);
};
